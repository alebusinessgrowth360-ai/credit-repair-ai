...
3. Extrae ABSOLUTAMENTE TODOS los hard inquiries — no omitas ninguno aunque sean muchos. Los inquiries son DIFERENTES a las cuentas: cada inquiry pertenece a UN solo buró específico (no se repiten en los 3 burós como las cuentas). Para cada inquiry incluye: empresa (nombre exacto), fecha (MM/YYYY), buro (TransUnion, Equifax, o Experian), tipo ("hard"), relacionado_con_cuenta (true si existe cuenta activa o cerrada con ese acreedor, false si no).

   CÓMO IDENTIFICAR EL BURÓ DE CADA INQUIRY:
   - Los reportes tienen secciones separadas: "TransUnion Inquiries", "Equifax Inquiries", "Experian Inquiries" (o variantes como "Inquiries - TransUnion"). Cada inquiry dentro de esa sección es de ese buró.
   - Si el texto plano muestra todos los inquiries juntos, busca el header de sección más cercano antes del inquiry para determinar a qué buró pertenece.
   - NUNCA repitas el mismo inquiry en 3 burós — cada empresa/fecha es una entrada única en UN buró.
   - NUNCA asignes TransUnion por defecto. Si no puedes determinar el buró, usa "Unknown".
