...
router.post('/evaluacion/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const rescores = await pool.query(
      'SELECT * FROM rapid_rescore WHERE reporte_id = $1 ORDER BY created_at ASC',
      [reporteId]
    )
    ...
    const rescoreList = rescores.rows || []
...
${rescoreList.length > 0 ? `
<h2>Rapid Rescore Analysis (${rescoreList.length} account${rescoreList.length !== 1 ? 's' : ''})</h2>
<table>
  <tr><th>Bank / Creditor</th><th>Account #</th><th>Type</th><th>Action</th><th>Current</th><th>Estimated</th><th>Impact</th><th>Timeline</th></tr>
  ${rescoreList.map((rs: any) => `
  <tr>
    <td>${rs.banco || ''}</td>
    <td>${rs.numero_cuenta || '—'}</td>
    <td>${rs.tipo_cuenta || ''}</td>
    <td>${rs.accion || ''}</td>
    <td style="font-weight:bold">${rs.score_actual || '—'}</td>
    <td style="font-weight:bold;color:#059669">${rs.score_estimado || '—'}</td>
    <td style="color:#059669;font-weight:bold">+${rs.impacto_puntos || 0} pts</td>
    <td>${rs.tiempo_estimado || '—'}</td>
  </tr>
  <tr><td colspan="8" style="font-size:12px;color:#374151;padding:4px 12px 12px"><strong>Analysis:</strong> ${rs.explicacion || ''} &nbsp; <strong>Recommendation:</strong> ${rs.recomendacion || ''}</td></tr>`).join('')}
</table>` : ''}
