  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0a0f1e', color:'#f1f5f9', fontFamily:'sans-serif', padding:'40px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'14px', marginBottom:'32px' }}>
        {[
          { label:'Total clientes', value: data?.total_clientes ?? 0 },
          { label:'Casos activos', value: data?.clientes_activos ?? 0 },
          { label:'Reportes este mes', value: data?.reportes_este_mes ?? 0 },
          { label:'Cartas generadas', value: data?.cartas_generadas ?? 0 },
          { label:'Disputas pendientes', value: data?.disputas_pendientes ?? 0 },
        ].map(m => (
          <div key={m.label} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'20px' }}>
            <div style={{ fontSize:'28px', fontWeight:600, color:'#6366f1' }}>{m.value}</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:'10px' }}>
        {[
          ...
          <Link key={a.href} href={a.href} style={{ textDecoration:'none' }}>
            <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'12px', padding:'18px', color:'#f1f5f9', fontSize:'13px', fontWeight:600 }}>
              {a.label} →
            </div>
          </Link>
        ))}
